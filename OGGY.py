...
class Get_Data:
    ...
    def get(self):
        ...
        if MY_KEY in respone:
            ...
            clear();print(f"{A}[{R}={A}] {G}USERNAME{R} : {A}MR ATIK");print(f"{A}[{R}={A}] {G}KEY {R}:{A} {MY_KEY} ");linex();print(f'{A}[{R}={A}] {G}PAID FF ID CLONE ');linex();print(f"{A}[{R}={A}] {G}FF ID CLONE 400 BDT {G}|{A} UNLIMITED  [600 BDT] {G} PEYMENT NOGOD");linex();print(f"{A}[{R}={A}] {G}1 SEND MONY AND USE THE TOOLS");linex();print(f"{A}[{R}={A}] {G}FF WORK");linex();print(f"{A}[{R}={A}] {G}400 TAKA ");linex();print(f"{A}[{R}={A}] {G}WHATSAPP NUMBER{R} >> {G}+8801608032030");linex();print(f"{A}[{R}={A}] {G}FACEBOOK ID    {R} >> {G}DABIT SULTAN ATIK");linex();input(f"{A}[{R}={A}] {G}TELEGRAM ID    {R} >> {G}@cybx90");linex()
            sys.exit()
import os
class Main:
